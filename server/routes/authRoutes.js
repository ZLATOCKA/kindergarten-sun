...
router.post('/register', async (req, res) => {
    const { email, password, name, phone, role, address, passport, birthDate, positionId, branchId } = req.body;
    ...
    try {
        ...
        if (role === 'parent') {
            // Для родителя обязательны адрес и паспорт
            if (!address || !passport) {
         ...
         RETURNING "Id_Родителя"`,
            ...
            return res.json({
                token,
                user: { id: result.rows[0].Id_Родителя, name, role: 'parent', email, phone }
            });
        }
        else if (role === 'employee') {
            // Для сотрудника обязательны адрес, дата рождения, должность и филиал
         ...
         ("Фамилия", "Имя", "Телефон", "Адрес", "Дата_рождения", "Email", "PasswordHash", "ID_Должности", "ID_Филиала")
         VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9) 
         RETURNING "ID_Сотрудника"`,
                [name.split(' ')[0] || '', name.split(' ')[1] || '', phone, address, birthDate, email, hashedPassword, positionId, branchId]
            );

            const token = jwt.sign({ id: result.rows[0].ID_Сотрудника, role: 'employee' }, process.env.JWT_SECRET, { expiresIn: '24h' });
            return res.json({
                token,
                user: { id: result.rows[0].ID_Сотрудника, name, role: 'employee', email, phone }
            });
        }
        else {
...
router.post('/login', async (req, res) => {
    ...
    try {
        if (role === 'employee') {
            ...
            return res.json({
                token,
                user: { id: user.ID_Сотрудника, name: `${user.Фамилия} ${user.Имя}`, role: 'employee', email, phone: user.Телефон }
            });
        }
        else if (role === 'parent') {
            ...
            return res.json({
                token,
                user: { id: user.Id_Родителя, name: `${user.Фамилия} ${user.Имя}`, role: 'parent', email, phone: user.Телефон }
            });
        }
